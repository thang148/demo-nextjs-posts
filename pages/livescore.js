...
const BoxTimeLiveScore = dynamic(() => import("components/BoxTimeLiveScore"))
...
export default function LiveScore({ topNews }) {
  ...
  async function fetch(date) {
    ...
    try {
      setLoading(true)
      const { data } = await apiLiveScore.getLiveSocres(convertTime(date))
      if (data?.length > 0) {
        __list.current = data
        setRows(data)
      } else {
  ...
  return (
    <main className="max_layout p-0 md:p-4 md:my-8">
      <div className="grid grid-cols-3 gap-6 ">
        <div className="col-span-3 tb:col-span-2 relative px-4 bg-white py-4">
          <Loading loading={loading}>
            ...
            <div className="mt-6">
              {row?.length > 0 ? (
                row?.map(({ name, logo, matchs }, k) => (
                  <div key={k} className="mb-6">
                    <div className="mb-4 flex items-center">
                      ...
                      <span className="font-bold ml-2 text-basic">{name}</span>
                    </div>
                    {matchs?.length > 0 &&
                      matchs?.map((item, x) => {
                        const {
                        ...
                        return (
                          <div
                            key={x}
                            className="text-darker grid grid-cols-5 bg-gray-50 rounded mb-2 py-2 cursor-pointer hover:bg-gray-200"
                          >
                            ...
                            <Link href={`detail-livescore/${id}`}>
                              <div className="col-span-1 ">
                                ...
                                {video?.status === "live" && (
                                  ...
                                  >
                                    <div className="w-2 h-2 rounded-full bg-red mr-1"></div> LIVE
                                  </button>
                                )}
                                {/* <BoxStatus status={time_status} renderLink={renderLink} /> */}
                              </div>
                            </Link>
                          </div>
                        )
                ...
                <div className="border border-gray-100 text-gray-400 text-sm p-4 h-32 text-center">
                  Không có trận đấu trong ngày hôm nay
                </div>
              )}
            </div>
          </Loading>
...
export async function getServerSideProps() {
  ...
  try {
    const response = await getPropsCategory()
