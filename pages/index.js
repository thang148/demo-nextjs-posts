...
import { getPropsHomePage } from "lib/dataStore"
export default function IndexPage({ screenblocks }) {
  if (!screenblocks?.length) return <ReLoad url="/" />
  useEffect(() => {
      ...
      <div className="max_layout">
        {screenblocks?.length > 0 &&
          screenblocks.map((screenblock, k) => {
            ...
            if (k === 3) {
              return (
                <div key={98}>
                  ...
                  <BoxRender screenblock={screenblock} key={k} />
                </div>
              )
            }
            // if (k === 4) {
            ...
            //   )
            // }
            return <BoxRender screenblock={screenblock} key={k} />
          })}
...
export async function getStaticProps() {
  ...
  try {
    pageProps = await getPropsHomePage()
  } catch (error) {
    console.log(error)
  }

  return { props: { ...pageProps }, revalidate: 30 }
}
