...
export default function Input({ type = "tel", error, ...rest }) {
  const [show, setShow] = useState(true)

  function onChangeShow() {
    setShow((c) => !c)
  }
  const isPassword = type === "password" && show
  return (
    <Fragment>
      <div className="relative">
        <input
          autoComplete="off"
          className="border-b border-gray-500 outline-none text-lg py-1 w-full bg-transparent"
          {...rest}
          type={isPassword ? "password" : "tel"}
          maxLength={isPassword ? 20 : 10}
        />
        {type === "password" && (
          <div
