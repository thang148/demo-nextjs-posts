...
export default function BoxTimeLive({ fetch }) {
  ...
  function onToday() {
    if (Math.abs(diffDate(listDates[14], toDay())) > 14) {
      ...
      if (window.innerWidth < 767) {
        ...
        __slider.current.scrollLeft = 750
      }
    }
    setSelected(toDay())
    fetch(toDay())
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      console.log("__slider.current.scrollLeft", __slider.current.scrollLeft)
    }, 300)
  }, [listDates])
  ...
  // console.log(__slider.current.scrollLeft)
  return (
    <div>
      <div className="flex justify-between">
        <div className="flex items-center mb-4">
          <div className="mr-4 text-2xl text-darker font-bold items-center flex">LIVESCORE </div>
          <div className={s.s2}>
      ...
      <div className="flex w-full">
        <div className="flex items-center justify-center">
          <button
            onClick={onToday}
            className={cn("border border-gray-300 text-basic rounded text-xs p-1 px-2 mr-2 md:mr-8", {
              "bg-gray-200": selected.getDate() === toDay().getDate()
            })}
          >
            HÔM NAY
          </button>
              ...
              return (
                <button
                  ...
                  className={cn("text-sm p-2 text-center", {
                    [`font-bold`]: item.getDate() === selected.getDate()
                  })}
